...
function formatResponse(queryResult: any, analyzedQuery: any): NLResponse {
	...
	switch (analyzedQuery.type) {
		case 'inspiration':
			response.summary = `I've found some interesting destinations you might like to visit${
				analyzedQuery.timeFrame.value
					? ` during ${analyzedQuery.timeFrame.value}`
					: ''
			}${
				analyzedQuery.budget
					? ` within your budget of ${analyzedQuery.budget.amount}`
					: ''
			}.`;
			break;

		case 'specific_route':
			response.summary = `Here are the best flight options from ${
				analyzedQuery.origin?.raw
			} to ${analyzedQuery.destinations[0]?.raw}${
				analyzedQuery.timeFrame.value
					? ` on ${analyzedQuery.timeFrame.value}`
					: ''
			}.`;
			break;

		case 'multi_city':
			response.summary = `I've planned a multi-city trip visiting ${analyzedQuery.destinations
				.map((d) => d.raw)
				.join(', ')}${
				analyzedQuery.duration
					? ` over ${analyzedQuery.duration.value} ${analyzedQuery.duration.type}`
					: ''
			}.`;
			break;
		...
		case 'flexible_value':
			response.summary = `I've found the most economical travel options for your trip${
				analyzedQuery.destinations.length > 0
					? ` to ${analyzedQuery.destinations[0].raw}`
					: ''
			}.`;
			break;
