...
server.tool(
  ...
  async (slots, _extra) => {
    ...
    try {
      ...
      // 최종 JSON 포맷
      const result = {
        message:       `${slots.departureDate} ${slots.originIataCode}→${slots.destinationIataCode} ` +
                       `편도 최저 ${priceMetrics[0].amount.toLocaleString()} ${slots.currencyCode}, ` +
                       `최고 ${priceMetrics[4].amount.toLocaleString()} ${slots.currencyCode} 사이에요.`,
      ...
      return {
        content: [
          {
            type: 'text',
            text: `가격 분석 중 오류가 발생했어요: ${error instanceof Error ? error.message : 'Unknown error'}`,
          },
        ],
        isError: true,
      };
